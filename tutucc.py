...
class Parser:
    ...
    def unary(self):
        ...
        if self.consume(TokenType.BITNOT):
            return self.new_unary(NodeKind.ND_BITNOT, self.cast(), token)
        return self.postfix()
    ...
    def add_type(self, node):
        ...
        if node.kind == NodeKind.ND_PTR_ADD or \
           ...
           node.kind == NodeKind.ND_DIV_EQ or \
           node.kind == NodeKind.ND_BITNOT:
            node.ty = node.lhs.ty
            return
...
class CodeGen:
    ...
    def gen(self, node):
        ...
        if node.kind == NodeKind.ND_NOT:
            ...
            print("  push rax")
            return
        if node.kind == NodeKind.ND_BITNOT:
            self.gen(node.lhs)
            print("  pop rax")
            print("  not rax")
            print("  push rax")
